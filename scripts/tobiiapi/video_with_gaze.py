...
this script was probably designed to be used with gstreamer 0.1
"""
...
class Video():
    ...
    # These are the different gstreamer components. Gstreamer uses ! between them to indicate which is which
    #
    # _PIPEDEF=[
    ...
    #     "ffdec_h264 max-threads=0",     # decode the incoming stream to frames - the actual decoding
    ...
    def start(self, peer, buffersync):
        ...
        print(self._bus)
        bus.add_watch(0, self._bus)

        # Set socket to get data from out socket
        # This passes the data from the socket to the "src" keyword to udpsrc gstreamer package
        # In the pipeline
        # gst-launch-1.0 -v udpsrc name=src blocksize=1316 closefd=false
        # buffer-size=5600 ! tsparse ! tsdemux emit-stats=True ! queue !
        # ffdec_h264 max-threads=0 ! identity name=decoded ! textoverlay
        # name=textovl text=* halignment=position valignment=position xpad=0
        # ypad=0 ! xvimagesink force-aspect-ratio=True name=video

        # the line below "src" needs to match the name we put in the udpsrc name

        # This works instead
        # gst-launch-1.0 -v udpsrc name=src blocksize=1316 close-socket=false buffer-size=5600 !
        # tsparse ! tsdemux emit-stats=True ! queue ! avdec_h264 max-threads=0 !
        # identity name=decoded ! textoverlay name=textovl text=*
...
if __name__=="__main__":
    ...
    glib.threads_init()
    ml = glib.MainLoop();
    peer1 = ("192.168.1.101", 49152)

    # rtsp://192.168.1.101:8554/live/scene # this doesn't give us the timestamps
    # peer2 = ("192.168.1.101", 8554)
    ...
    et.start(peer1, buffersync)
    video.start(peer1, buffersync)
    # Enter main loop
    ml.run()
